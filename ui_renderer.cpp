...
void updateVisualizerAnimation() {
  ...
  if (now - lastVisAnimMs < VISUALIZER_REFRESH_MS) return;
  lastVisAnimMs = now;
...
void drawStartupScreen() {
  ...
  if (!tryDrawSplashFromSD(rawX, rawY)) drawGuaraLogoProcedural(innerX, innerY, innerW, innerH);
...
void drawBluetoothPicker(bool connecting) {
  tft.fillScreen(COL_BG);
  ...
  for (int row = 0; row < vis; row++) {
    ...
    int rssi = -128;
    if (!btGetScanEntry(idx, nmCopy, sizeof(nmCopy), &rssi)) continue;
